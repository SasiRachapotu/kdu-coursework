...
} from "../../styles/portfoliostyles/portfolio.style";
import Filters from "./Filters";
import {
  amountStyles,
  dataBlockStyles,
  dateHeadingStyle,
  individualStockStyles,
  stockNameStyles,
  stockSymbolStyles,
  timeStatusStyles,
} from "../../styles/portfoliostyles/individualStock.style";
...
export function PortfolioContainer() {
  const statusList = useSelector(
    (store: RootState) => store.portfolio.statusList
  );
  const filterStocksList = useSelector(
    (store: RootState) => store.portfolio.filterStocksList
  );
  const filterInput = useSelector(
    (store: RootState) => store.portfolio.filterInput
  );
  const startDate = useSelector(
    (store: RootState) => store.portfolio.startDate
  );
  const endDate = useSelector((store: RootState) => store.portfolio.endDate);
  const dateFilter = useSelector(
    (store: RootState) => store.portfolio.dateFilter
  );
  const currentTransactions = useSelector(
    (store: RootState) => store.transactions.currentTransactions
  );

  const groupedTransactions = useSelector<{
    [date: string]: IParsedResponse[];
  }>((store: RootState) => store.portfolio.groupedTransaction);
    ...
    <div className="main-container" style={portfolioContainerStyle}>
      ...
      <RightContainer>
        {Object.entries(groupedTransactions).map(
          ([date, transactionsOfDay]) => {
            // Filter transactions based on various conditions
            const filteredTransactions = transactionsOfDay.filter(
              (transaction) => {
                const statusFilter =
                  statusList.length === 0 ||
                  statusList.includes(transaction.status);
                const filterStocksFilter =
                  filterStocksList.length === 0 ||
                  filterStocksList.includes(transaction.stock_name);
                            ...
                            {filteredTransactions.map((transaction) => (
                              ...
                              >
                                ...
                                <div
                                  ...
                                  style={stockNameStyles}
                                >
                                  {transaction.stock_name}
                                </div>
                                <div
                                  className="stock-symbol"
                                  style={stockSymbolStyles}
                                >
        ...
        <div key="12345" className="date-block" style={dataBlockStyles}>
          <div className="date-heading" style={dateHeadingStyle}>
            {currentTransactions[0]?.date instanceof Date? currentTransactions[0]?.date.toISOString().split("T")[0]:"6:00 AM"}
          </div>
          {currentTransactions.map((transaction) => {
            return (
              <div
                key={transaction.stockName}
                className="individual-stock"
                style={individualStockStyles}
              >
                {/* Render transaction details */}
                ...
                <div className="stock-symbol" style={stockSymbolStyles}>
                  {transaction.stockName.substring(0,3)}
                </div>
                <div className="amount" style={amountStyles}>
                  ₹{5666}
                </div>
